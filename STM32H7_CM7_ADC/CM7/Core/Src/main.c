...
// 1024 samples in D2 (non-cacheable on H7 by default)
uint16_t adc_buf_1[1024];
...
/* Two-sample buffer in rank order: [0]=CH5(PC0), [1]=CH4(PA0) */
RAM_D2_NL volatile uint16_t adc_buf[3] = {0};
HAL_StatusTypeDef ADC1_Status, TIM3_status;
...
void HAL_MDMA_FullCpltCallback(MDMA_HandleTypeDef *hdma)
{
    if (hdma == &hmdma_mdma_channel0_dma1_stream0_tc_0) {
        SCB_InvalidateDCache_by_Addr((uint32_t*)ch6_buf, sizeof(ch6_buf));
        SCB_InvalidateDCache_by_Addr((uint32_t*)ch5_buf, sizeof(ch5_buf));
        mdma_full_ready = 1;   // safe for CPU to read both
    }
}
...
uint32_t GetTIM3Clock(void)
{
    uint32_t pclk1 = HAL_RCC_GetPCLK1Freq();
    /* If APB1 prescaler is not DIV1, timer clock is 2x PCLK1 */
...
uint32_t TIM3_GetCounterClockHz(void)
{
    uint32_t timclk = GetTIM3Clock();
    uint32_t psc    = 63u; // reads TIM3->PSC
    return timclk / (psc + 1U);
}

uint32_t TIM3_GetUpdateEventHz(void)
{
    uint32_t ck_cnt = TIM3_GetCounterClockHz();
    uint32_t arr    = __HAL_TIM_GET_AUTORELOAD(&htim3); // reads TIM3->ARR
    return ck_cnt / (arr + 1U);
}
